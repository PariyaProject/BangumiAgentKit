...
官方评分/排名/评分人数/收藏总数、评分直方图样本/均值/总体标准差、收藏分布和观察完成率及 `B − A` 数值差；同时按两侧本次有界官方 v0 角色和职员
...
没有观察到共同人物”，差值和共同人物不代表推荐、胜负或观看顺序。统计分布保留
official-v0 快照、derived-s7 公式版本和 `not_computable` / `partial` / `unavailable` 状态，
不将缺失值补成零或生成推荐。人类输出有行数、字符数和
