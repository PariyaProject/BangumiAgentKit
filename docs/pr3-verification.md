...
| P3-06 | Shared RuntimeDependencies | `packages/tools/src/registry.ts` (`createRuntimeDependencies`) | `tests/integration/cross-instance-persistence.test.ts` | VERIFIED |
| P3-07 | Remove ToolContext.accessToken | `packages/tools/src/define-tool.ts` | `tests/contract/compile-negative.test.ts` | VERIFIED |
| P3-08 | TokenBroker client boundary | `packages/auth/src/token-broker.ts` | `tests/unit/capability-policy.test.ts` | VERIFIED |
| P3-09 | Optional/public/authenticated client | `TokenBroker.getPublicClient`, `getOptionalAuthenticatedClient`, `requireAuthenticatedClient` | `tests/unit/capability-policy.test.ts` | VERIFIED |
| P3-10 | OAuth requested capability != granted scope | `packages/auth/src/oauth-service.ts` (`reportedScopes`, `scopeEvidence`) | `tests/unit/capability-policy.test.ts` | VERIFIED |
...
| P3-13 | Refresh token implementation | `TokenBroker.requireAuthenticatedClient` | `tests/integration/postgres-refresh-concurrency.test.ts` | VERIFIED |
| P3-14 | Concurrent refresh deduplication | `TokenBroker.withCredentialLock` | `tests/integration/postgres-refresh-concurrency.test.ts` | VERIFIED |
| P3-15 | AES-GCM key version | `packages/auth/src/token-crypto.ts` (`TokenKeyring`, `encryptToken`, `decryptToken`) | `tests/unit/auth.test.ts` | VERIFIED |
| P3-16 | Actual Fastify OAuth callback | `apps/api/src/app.ts` & `routes/oauth.ts` | `tests/integration/fastify-api.test.ts` | VERIFIED |
| P3-17 | auth_status | `packages/tools/src/definitions/auth-tools.ts` (`bangumi.auth_status`) | `tests/integration/cross-instance-persistence.test.ts` | VERIFIED |
...
| P3-20 | Dynamic ResolvedToolPolicy | `packages/tools/src/policy.ts` / `defineTool.resolvePolicy` | `tests/unit/manage-index-actions.test.ts` | VERIFIED |
...
| P3-25 | Write audit | `packages/bangumi-core/src/services/audit-service.ts` | `tests/unit/audit-account.test.ts` | VERIFIED |
