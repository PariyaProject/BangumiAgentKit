...
| **Person activity/workload gap (G04/G05)**                        | High in principle, but the broad workload/history question remains unsupported.               | `bangumi.get_person_activity` already bounds relations to 120, subject details to 48, rows to 60, and detail concurrency to 4; it uses `first_air_date` as the window key.                                                                                                                                    | Public v0 person data needs no account. The honest boundary is “observed participation by release-date window,” not labor time, workload intensity, or historical growth. No v0 snapshot/event stream supports a retrospective workload comparison. | Agent and Renderer coverage already exist with explicit missing-date/media/role/detail-failure semantics and tests. A bounded current-relation release-date comparison is a safe salvage only when it preserves coverage states and never calls itself workload/history. | **Research-ready for workload/history, not for the bounded salvage.** The current Epoch may ship the narrower two-window release-date observation; a history/workload comparison still requires a separate local observation, retention, and consent contract. |
...
The remaining broad question — “did this person do more work in one period than another?” — is not closed by v0. `first_air_date` is a release-date proxy, not a work-date or workload measure, and v0 supplies no historical snapshot series. That workload/history interpretation remains **RESEARCH_READY** and needs a consented local statistics-observation contract. A narrower safe salvage is different: compare two equal current-relation windows by observed `first_air_date`, preserve each period’s coverage/degraded state, and label the result as a release-date observation rather than workload or history. The current Epoch uses only that narrower salvage; it does not supersede the research blocker for the broad question.
