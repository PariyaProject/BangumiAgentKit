...
Renderer 2.0 的数据模型必须把“有值”“来源稳定”“允许展示”“可计算”分开。S1/S2 可直接支撑 SubjectOverview、Cast、Staff、Episodes、Relations 和基础 Calendar；Stats、CommunityTrending、PersonWorkload、CollectionDashboard 需要分别表达 S5/S3/S6/auth 状态。
...
**REASONING**：不同 source class 的值不可只靠颜色/数字区分；同一个卡片可能同时有 official score、HTML histogram、derived workload 和 private progress。
...
| SubjectDeepDive     | S1 episodes/cast/staff/relations；S3 topics/reviews | stats/正文/coverage                        | S1 + S3/S5 per section                  |
...
| Stats               | score/count from S1/S2                              | histogram/completion currently S5          | S5 + S7; display unavailable explicitly |
