...
| Bucket   | 定义                                                                        |    数量 |   占 103 |
| -------- | --------------------------------------------------------------------------- | ------: | -------: |
| `API+S7` | S1/S2 原始数据足够，允许 S7 join/filter/graph；括号 `auth` 表示需要用户授权 |      81 |   78.64% |
| `S3+S7`  | 需要官方 frontend private structured read；S5 可作为显式 fallback           |      13 |   12.62% |
| `S5+S7`  | 仍需网页 HTML 的 website-specific stats/分布                                |       0 |    0.00% |
...
| G08 | API+S7        | S1 Subject rating histogram/collection buckets + S7 completion definition        |
...
| G15 | API+S7        | S1 score/eps/histogram/collection + S7 completion comparison                     |
...
| G23 | API+S7        | S1 histogram + S7 mean/population SD/sample size; S5 only if web chart requested |
...
| ID  | Bucket | 最小 evidence / 关键限制                                             |
| --- | ------ | -------------------------------------------------------------------- |
| A01 | API+S7 | S1 histogram + S7 population SD + season cohort/sample size          |
...
| A05 | API+S7 | S1 score/collection buckets + S7 completion rate                     |
...
**FACT**：按上述最小证据集合，81 个场景只需要 S1/S2 + S7，13 个需要 S3，0 个场景将 S5 作为当前最小原始输入，8 个需要 S6，1 个当前 S8。
...
**ALTERNATIVES**：若未来 S3 提供稳定 website-specific stats，S5 可从 fallback 进一步后移；若没有快照，8 个 S6 场景必须保持 not-computable，不得降级成 current snapshot。
