...
const calendarProvider: CalendarProvider = {
  async getCalendar() {
    return { state: 'ok', data: [], coverage: { state: 'complete' }, retrievedAt: '2026-08-09T00:00:00Z' };
  },
};
...
describe('PR-7B provider registry and safe diagnostics', () => {
  it('exposes only configured official capabilities and explicit disabled statuses', async () => {
    const registry = new ProviderRegistry({ v0: subjectProvider, legacyCalendar: calendarProvider });
    ...
    expect(JSON.stringify(registry.getDiagnostics())).not.toMatch(/token|header|principal|database/i);
  });
  ...
  it('does not turn an absent provider into an empty success', async () => {
    ...
    expect(result.warnings?.[0]?.code).toBe('SOURCE_DISABLED');
  });
