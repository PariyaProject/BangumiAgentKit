...
export function createReadTools(clientProviderOrHttpClient?: BangumiClientProvider | HttpClient) {
  ...
  const getPersonActivity = defineTool({
    ...
    description:
      '按官方 v0 人物关系与有界作品详情计算指定时间窗内的声优/制作人员 activity。保留原始角色或职位标签，按作品 first_air_date 归入日历月；达到关系或详情预算时在官方返回顺序上做确定性等距抽样，并显式报告媒介筛选、缺日期、未知角色、详情失败、观察/选取/省略 ID 和各项预算。可选 comparePreviousWindow=true 计算最近窗口与紧邻等长窗口的作品/角色数量差值和观察到的发布月份峰值；这仍基于当前关系与作品首播日期，不是历史快照、劳动时长或实际配音时间。',
    input: z
      .object({
        personId: z.number().int().positive().describe('Bangumi 人物 ID'),
        kind: z
        ...
        comparePreviousWindow: z
          .boolean()
          .optional()
          .describe('是否比较最近窗口与紧邻的等长日历月窗口；默认 false，不使用历史快照'),
      })
      .strict(),
    auth: 'none',
    scopes: [],
