...
| `bangumi.get_subject_stats_intelligence` | none            | read     | 官方评分直方图/收藏桶及有版本的分布、均值、总体标准差、完成率与证据状态                                  |
...
| `bangumi.get_person_collaboration`       | none            | read     | 基于官方 v0 共同作品关系按稳定人物/作品 ID 排名合作人物，保留共同作品、原始职位/角色、fan-out 和省略覆盖 |
...
`bangumi.render_subject_stats_intelligence`, `bangumi.render_series_watch_order`, and
...
The subject-stats renderer is image-free and read-only. It displays the official
v0 snapshot beside derived-s7 percentage, mean, population-standard-deviation,
and completion formulas, along with source operations, coverage, conflict
candidates, zero-population `not_computable`, and unavailable/not-found states.
It does not fetch image assets, render comments, inspect community/history
sources, or turn dispersion into a quality or recommendation score.
...
The person-collaboration semantic tool and renderer are public, read-only, and
bounded. They fan out from official v0 person-to-subject/character relations to
official subject-to-person/character relations, deduplicate by stable person and
subject ID, and rank collaborators by observed shared-subject count. Literal
target-role and staff collaborator-role filters preserve source labels; voice
actor responses do not expose a collaborator role, so `collaboratorRole` never
uses an actor's career as a substitute. Relation, subject, collaborator, shared
subject, concurrency, failure, malformed-row, self-row, and output coverage are
returned explicitly. The result is not a complete industry graph, historical
trend, workload measure, or recommendation.
