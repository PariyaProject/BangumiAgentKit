...
function presentPersonActivity(value: Record<string, unknown>): string | undefined {
  ...
  for (const [index, rawRow] of rows.slice(0, 12).entries()) {
    ...
    const rawRole =
      typeof row.rawRole === 'string' && row.rawRole.trim() ? row.rawRole : '未知（来源未提供）';
    lines.push(
      `${index + 1}. ${humanField(row.subjectNameCn || row.subjectName || `条目 ${row.subjectId || '?'}`, 180)} #${humanField(row.subjectId ?? '?', 32)} · ${humanField(row.firstAirDate || '日期未知', 32)} · ${row.relationKind === 'voice' ? '声优' : row.relationKind === 'staff' ? '制作人员' : humanField(row.relationKind || '关系未知', 32)} · ${humanField(row.roleFamily || '职位未知', 64)} · 原始职位/角色：${humanField(rawRole, 160)} · 来源观察：${labelOrigin(rowOrigin?.state)}${tags ? ` · ${tags}` : ''}`,
    );
  }
